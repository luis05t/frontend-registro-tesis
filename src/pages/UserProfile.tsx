...
const RegisterPage = () => {
  ...
  useEffect(() => {
    const fetchCareers = async () => {
      try {
        // CORRECCIÓN 1: Agregamos ?limit=1000 para traer TODAS las carreras
        const res = await api.get('/api/careers?limit=1000')
        // Aseguramos leer correctamente si viene en .data directo o .data.data
    ...
    const fetchRole = async () => {
      try {
        // CORRECCIÓN 2: Agregamos ?limit=1000 para roles también (por si acaso)
        const res = await api.get('/api/roles?limit=1000')
        const rawData = res.data;
        const data = Array.isArray(rawData) ? rawData : (rawData.data || []);
        setRole(data)
      } catch (error) {
