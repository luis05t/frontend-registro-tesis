...
const UserProfile = () => {
  ...
  const loadProfile = async () => {
    ...
    try {
      ...
      // CORRECCIÓN AQUÍ: Agregamos ?limit=1000 para traer TODAS las carreras y roles
      // Si no ponemos esto, la API solo trae las primeras 10 y si tu carrera es la 11, sale "Sin asignar"
      const [careersRes, rolesRes] = await Promise.all([
        api.get("/api/careers?limit=1000"),
        api.get("/api/roles?limit=1000")
      ])

      // Verificación de seguridad para leer los datos (ya sea array directo o paginado en .data)
